...
/**
 * Validation middleware. Handlers never see unvalidated input — a route declares
 * schemas for body/query/params and receives typed, parsed values.
 *
 * On failure we return 422 with Zod's field-level details, so the browser form
 * (validating with the SAME shared schema) and the server agree on error shape.
 */
