 ...
 * header themselves.
 *
 * `req.validated` is populated by validate(). It is typed `unknown` on purpose:
 * handlers narrow it with the same Zod-inferred type the route declared, which
 * keeps the schema the single source of truth instead of letting a loose `any`
 * drift away from it.
 */
declare global {
  namespace Express {
